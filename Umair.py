...
# Global Variables
team_name = 'Shields'                        # The Name of the Team
...
fo = open('storage/log-{%Y-%m-%d %H:%M:%S}'.format(datetime.datetime.now()) + '.txt', 'w')
...
def data_recording(parsed_json):
    ...
    fo.write(str(parsed_json) + '\n') 
    time_stamp = str(datetime.datetime.now()) 
    redis_db.set(time_stamp, str(parsed_json))
