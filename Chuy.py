...
# Global Variables
team_name = 'while(E){'                        # The Name of the Team
  ...
  """
    ...
    for team in teams_list:
        if team['name'] == team_name:
            if team['shield'] <> True and team['energy'] > 30:
                shield_up = team['shield']
                energy = team['energy']
                break
    ...
    radAvg = np.mean(radiation)
    #tmpAvg = np.mean(temperature)

    print 'Solar flare: ' + str(readings['solarFlare'])
    if readings['radiation'] < radAvg or readings['solarFlare'] == True:
        team_shield_up(team_name, team_auth)
    else:
        if readings['temperature'] > -30:
            team_shield_down(team_name, team_auth)

    radiation.append(readings['radiation'])
    temperature.append(readings['temperature'])
